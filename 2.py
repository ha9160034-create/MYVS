import pygame
from sys import exit
from random import randint

def display_time():
    current_time= int(pygame.time.get_ticks() / 1000) - start_time
    score_surf= test_font.render(f"Score: {current_time}",False,(64,64,64))
    score_rect=score_surf.get_rect(center= (400,50))
    screen.blit(score_surf,score_rect)
    return current_time

def obstacle_movement(obstacle_list):
    if obstacle_list:
        for obstacle_rect in obstacle_list:
            obstacle_rect.x -= 5
            
            if obstacle_rect.bottom == 389:screen.blit(snail_surf,obstacle_rect)
            else:screen.blit(owl_surf,obstacle_rect)

        obstacle_list = [obstacle for obstacle in obstacle_list if obstacle.x > -100]
        return obstacle_list
    else: return []

def collisions(player,obstacles):
    if obstacles:
        for obstacle_rect in obstacles:
            if player.colliderect(obstacle_rect): return False
    return True

def player_animation():
    global player_surf,player_index
    if player_rect.bottom < 380:
        player_surf=player_jump
    else:
        player_index += 0.1
        if player_index >= len(player_walk): player_index = 0
        player_surf = player_walk[int(player_index)]

pygame.init()

game_active = False

start_time = 0
score = 0

screen=pygame.display.set_mode((800,500))
clock=pygame.time.Clock()

test_sky=pygame.image.load("sky.png").convert()
sky= pygame.transform.scale(test_sky,(800,500))
sky_rect=sky.get_rect(topleft=(0,0))

test_font=pygame.font.Font(None,50)
# font_surface=test_font.render("aba neo",True,"Red")
# font_rect=font_surface.get_rect(midtop=(sky_rect.midtop))

ground_surface=pygame.image.load("ground.png").convert_alpha()
ground=pygame.transform.scale(ground_surface,(820,200))

snail_walk_1=pygame.image.load("snail_walk_1.png").convert_alpha()
snail_swalk_1=pygame.transform.scale(snail_walk_1,(59,59))
snail_walk_1_rect=snail_walk_1.get_rect(midbottom=(840,389))
snail_walk_2=pygame.image.load('snail_walk_2.png')
snail_walk_2=pygame.transform.scale(snail_walk_2,(59,59))
snail_walk_2_rect=snail_walk_2.get_rect(midbottom =(840,389))
snail_walk=[snail_swalk_1,snail_walk_2]
snail_walk_index = 0
snail_surf=snail_walk[snail_walk_index]

owl_walk_1=pygame.image.load("owl_walk_1.png").convert_alpha()
owl_walk_1=pygame.transform.scale(owl_walk_1,(70,70))
owl_walk_1_rect=owl_walk_1.get_rect(midleft=(500,120))
owl_walk_2=pygame.image.load('owl_walk_2.png')
owl_walk_2=pygame.transform.scale(owl_walk_2,(70,70))
owl_walk_2_rect=owl_walk_2.get_rect(midleft=(500,120))
owl_walk=[owl_walk_1,owl_walk_2]
owl_walk_index = 0
owl_surf=owl_walk[owl_walk_index]

obstacle_rect_list = []

player_walk_1=pygame.image.load("mario_walk_1.png").convert_alpha()
player_walk_2=pygame.image.load("mario_walk_2.png").convert_alpha()
player_walk_1=pygame.transform.scale(player_walk_1,(70,90))
player_walk_2=pygame.transform.scale(player_walk_2,(70,90))
player_walk= [player_walk_1,player_walk_2]
player_index= 0
player_jump=pygame.image.load('mario_jump.png').convert_alpha()
player_jump=pygame.transform.scale(player_jump,(70,90))

player_surf= player_walk[player_index]
player_rect=player_surf.get_rect(midbottom=(80,380))
player_gravity=0

player_stand=pygame.image.load('mario.png')
player_stand=pygame.transform.scale(player_stand,(90,110))
player_stand_rect=player_stand.get_rect(center=(380,250))

game_name=test_font.render("Pixel Runner",False,(111,196,169))
game_name_rect=game_name.get_rect(center= (390,150))

game_message=test_font.render("Press space to run",False,(111,196,169))
game_message_rect=game_message.get_rect(center=(390,350))

obstacle_timer=pygame.USEREVENT +1
pygame.time.set_timer(obstacle_timer,1500)

snail_timer=pygame.USEREVENT + 2
pygame.time.set_timer(snail_timer,500)

owl_timer=pygame.USEREVENT + 3
pygame.time.set_timer(owl_timer,200)

while True:
    for event in pygame.event.get():
        if event.type==pygame.QUIT:
            exit()
        if game_active:
            if event.type == obstacle_timer:
                if randint(0,1):
                    obstacle_rect_list.append(snail_surf.get_rect(midbottom = (randint(900,1100),389)))
                else:
                    obstacle_rect_list.append(owl_surf.get_rect(midleft = (randint(900,1100),250)))
            if event.type==snail_timer:
                if snail_walk_index == 0: snail_walk_index = 1
                else: snail_walk_index = 0
                snail_surf= snail_walk[snail_walk_index]
            if event.type==owl_timer:
                if owl_walk_index == 0: owl_walk_index = 1
                else: owl_walk_index = 0
                owl_surf= owl_walk[owl_walk_index]
        else:
            if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE :
                game_active =True
                snail_walk_1_rect.left = 800
                snail_walk_2_rect.left = 800
                start_time= int(pygame.time.get_ticks() / 1000)

    keys=pygame.key.get_pressed()
    if keys[pygame.K_RIGHT] and player_rect.right < 800:
        player_rect.right += 3
    if keys[pygame.K_LEFT] and player_rect.left > 0:
        player_rect.left -= 3
    if keys[pygame.K_SPACE] and player_rect.bottom >= 380 :
        player_gravity = -20

    if game_active:
        screen.blit(sky,sky_rect)

        screen.blit(ground,(-10,380))
        
        # snail_rect.left-=5
        # if snail_rect.right <= 0: snail_rect.left = 800
        # screen.blit(snail,snail_rect)
        
        player_gravity += 1
        player_rect.y += player_gravity
        if player_rect.bottom >= 380: player_rect.bottom = 380
        player_animation()
        screen.blit(player_surf,player_rect)
        
        obstacle_rect_list = obstacle_movement(obstacle_rect_list)

        # if player_rect.colliderect(snail_rect):
        #     print("collision")
        # mouse=pygame.mouse.get_pos()
        # if player_rect.collidepoint((mouse)):
        #     print("collision")
        
        # screen.blit(font_surface,font_rect)
        score=display_time()
        game_active = collisions(player_rect,obstacle_rect_list)
        # if snail_rect.colliderect(player_rect):
        #     game_active = False
    else:
        screen.fill((94,129,162))
        screen.blit(player_stand,player_stand_rect)
        obstacle_rect_list.clear()
        player_rect.midbottom = (80,380)
        player_gravity = 0
        screen.blit(game_name,game_name_rect)
        score_message=test_font.render(f"your score is: {score}",False,(111,196,169))
        score_message_rect=score_message.get_rect(center=(390,350))
        if score == 0: screen.blit(game_message,game_message_rect)
        else: screen.blit(score_message,score_message_rect)
    pygame.display.update()
    clock.tick(60 )
    her=print('hello')